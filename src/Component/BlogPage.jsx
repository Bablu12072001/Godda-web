...
const Blog = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${apiUrl}/jmoa_blog_all_data`);
    ...
    ? // Display skeleton while loading
      Array.from({ length: itemsPerPage }).map((_, index) => (
        ...
        >
          ...
          <Skeleton height={20} width="90%" style={{ marginTop: 5 }} />
        </div>
      ))
        ...
        .map((item) => (
          ...
          >
            ...
            <div className="text-left">
              ...
              <p style={{ color: "#454e81" }}>{item.content}</p>
              <Link
  ...
  return (
    <div className="text-center mt-10">
      <h1
        className="text-3xl font-bold mb-4 border-b-2 border-gray-500 pb-2 inline-block "
        style={{ color: "#652b7c" }}
      >
        New Blogs
      </h1>
